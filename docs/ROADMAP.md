...
- **CI: `make test-coverage` on every PR**: GitHub Actions workflow that runs `make test-coverage` and `bundle exec rspec` on every PR. Fail the build if any covered JS file drops below 100% line or 95% branch, and if Ruby drops below 100/100. Without this, coverage gains decay within a few commits — the only thing keeping the bar in place is "someone ran the survey this morning". Could also gate on `make verify` (HTTP-probe deployed URLs) as a post-merge job.
