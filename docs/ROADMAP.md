...
- **Bordered, stippled score progress bar**: the quiz's current top-of-screen score line ("Score: 3/4 of 24  ▓▓▓░░░░░░░░░░░░░") combines a filled-blocks-vs-light-blocks bar inline with the text. Future visual: a framed bar with a solid line border around it, where each slot is a stipple character (one per question), and the slots corresponding to answered-correct fill in as solid stipples. Communicates both *how many questions exist* and *how many are right so far* at a glance, with stronger affordance than the current inline bar. Implementation lives in `progressBar` (quiz `app.js`) and `renderScoreBanner`. Stippling pattern + border characters TBD — could be drawn with box-drawing characters (├─┤) or a CSS-bordered HTML element if we move the bar out of the `<pre>`.

- **Restore walkthrough Node test runners**: `web/walkthrough/run-tests-node.mjs` and `run-dag-cross-verify.mjs` are currently broken under Node 22+ — `tests.js` is a CommonJS module and the `.mjs` runners try to import named exports from it, which Node refuses. Fix by either converting `tests.js` to a real ESM module (export named functions directly) or by importing the default and destructuring (`import pkg from './tests.js'; const { runScenarios } = pkg;`). Once green, wire back into the `make test` target.

- **Fix `buildRandomQuestion` pool overrun**: In `web/quiz/quiz.js`, `buildRandomQuestion` picks `count = 1 + floor(rand * 5)` (so 1–5) but the field pool may be shorter. The partial-Fisher-Yates loop correctly clamps via `i < count && i < pool.length`, but the *assignment* loop after it only checks `i < count`, so when `count > pool.length` it overruns: `pool[i]` is `undefined`, and `inputs[undefined] = true` plants a phantom `"undefined"` key in the input bag. The trace produced by the rule engine then matches against a malformed bag. Fix: clamp the assignment loop too, e.g. `for (let i = 0; i < count && i < pool.length; i++)`. Test `web/quiz/test-quiz.mjs` currently documents the bug rather than asserting the fix.
...
- **Rule-driven regeneration of every derived asset**: Aspiration: editing `nasfaa_rules.yml` (or `nasfaa_questions.yml` / `nasfaa_scenarios.yml`) should be the *only* manual change required to ship a rule update — every dependent artifact regenerates from the YAML.

  **What's already data-driven** (no manual touching needed when YAML changes):
  - `Nasfaa::RuleEngine` — reads YAML directly.
  - All four CLI modes (walkthrough, quiz, evaluate) — driven by the YAML.
  - Web pages (`data.js` / `*.json` bundles) — regenerated by `make build`.
  - Exhaustive cross-engine verification (2^12 × 9 combinations).

  **What still requires hand-editing when rules change:**
  - `lib/nasfaa/decision_tree.rb` — the imperative `if/elsif` tree, the second independent implementation used for cross-verification.
  - `nasfaa_questions.yml` — the question DAG. Conceptually separate from rules, but rule changes often imply DAG changes (Box 5 / Box 8 fixes touched both).
  - `spec/support/paths.rb` — canonical compact y/n paths per terminal rule. Hand-curated.
  - Per-rule spec assertions in `spec/walkthrough_spec.rb`, `spec/rule_engine_spec.rb`, etc. (rule_id strings, path strings, count assertions like `expect(rules.length).to eq(21)`).
  - Narrative in `nasfaa_scenarios.yml` (when a rule's semantic shifts).
  - README + ROADMAP rule-count mentions.

  **Candidate approaches (not exclusive):**
  - **Compile `decision_tree.rb` from the YAML.** Loses the "two independent implementations" safety property; gains automatic regeneration. Could mitigate by committing a frozen reference version that the generated tree is diffed against in CI.
  - **Replace `decision_tree.rb` with property-based tests** (rantly / propcheck) that verify invariants directly, removing the need for a second implementation entirely.
  - **`make verify-derived`** that runs cheap consistency checks: rule count matches path-table cardinality, every rule has a scenario, every scenario rule_id is real, README's `21 rules` matches reality, etc. Doesn't auto-fix but flags drift loudly.
  - **`make regenerate`** (umbrella) that runs `make build` plus any other derived-asset regenerations and prints a list of files that changed.

  **First step:** enumerate every file/line that hard-codes rule-derived state (rule IDs, rule counts, path strings). The list above is sketched, not exhaustive. Once enumerated, decide per-class whether automation is worth the complexity.
...
- **Section 508 compliance audit**: Run a formal Section 508 conformance audit on both web pages, covering keyboard navigation (tab order, focus rings, `:focus-visible` on the `.prompt-key` spans — currently `<span>`s without `role` or `tabindex`), screen reader landmarks (proper `<main>`/`<nav>` semantics, `aria-label` on the dev badge + theme toggle, `aria-live` on the result reveal), color contrast verified beyond the WCAG-AA tokens already in `tokens.css` (run axe-core / Lighthouse / WAVE on each of the three themes), `prefers-reduced-motion` (scanlines + cursor blink already respect it; verify theme-cycle transitions do too), and an alt-text pass on decorative glyphs. The federal regulatory subject matter makes 508 alignment a strong candidate for a v0.2 release gate. Subsumes the keyboard/focus portion of the deferred UX-unification work; complements the existing "Web page styling refresh — accessible color system" entry.

- **Update quiz scenarios to not telegraph their answers**: Several scenarios in `nasfaa_scenarios.yml` include phrasing in the `description:` field that effectively gives away the expected `result:` (statute names, "permitted"/"not allowed" verbs, etc.). A quiz where the question contains the answer isn't really a quiz. Audit each of the 24 scenarios; rewrite descriptions to present the *situation* without naming the conclusion or the governing body. Reveal copy after the answer can keep the explanatory language. Overlaps with the existing "Quiz scenario accuracy deep dive" entry — could be bundled into that pass or done as a narrower, faster sweep first.

- **Mobile / small-viewport responsive audit**: Both pages got tap-friendly prompt keys this session but the end-to-end small-screen behavior hasn't been audited. Test below 600px and below 400px viewports for: box overflow handling (`overflow-x: auto` saves us functionally but visually is awkward — consider responsive font-size or reflow), dev-badge placement (`fixed; top-right` may overlap the theme toggle on narrow screens), build footer + theme toggle + shamrock link not overlapping each other or the centered stack, banner-subtitle line breaks at narrow widths. Complements the existing "Mobile / browser test matrix" entry (cross-browser smoke pass) by going deeper on the small-viewport CSS specifically.

- **Walkthrough Paper → Terminal style convergence**: User declared gate #1 "sufficient for now" on 2026-05-24, but convergence isn't complete — the walkthrough is still document-flow inside `.terminal` while the quiz has a bordered/elevated terminal frame. Full convergence would put the walkthrough's question + result boxes inside a single outer terminal frame mirroring the quiz's chrome. Picks up when v0.2 polish work resumes.

- **CI: `make test-coverage` on every PR**: GitHub Actions workflow that runs `make test-coverage` (and `bundle exec rspec` once the Ruby bundle is fixed — see below) on every PR. Fail the build if any covered JS file drops below 100% line or 95% branch. Without this, the JS coverage gains decay within a few commits — the only thing keeping the bar in place is "someone ran the survey this morning". Could also gate on `make verify` (HTTP-probe deployed URLs) as a post-merge job.

- **Fix Ruby `bundle install`**: As of 2026-05-24, `bundle exec rspec` fails with `Could not find rake-13.3.1, racc-1.8.1 in locally installed gems`. The Gemfile.lock pins versions that aren't available in the current rvm gem cache. Either run `bundle install` to refresh or update the lockfile to gem versions installable from the local cache. Blocks SimpleCov refresh and any Ruby-side coverage work — `bin/coverage-survey`'s Ruby table is stale-by-default until this is fixed.

- **Rule provenance: PDF page references on each rule**: Each rule in `nasfaa_rules.yml` has a `citation:` pointing at the statute (e.g., "IRC §6103(l)(13)"), but not at the location in the canonical NASFAA PDF (`docs/NASFAA_Data_Sharing_Decision_Tree.pdf`) the rule was extracted from. Add `source: { pdf_page: 7, pdf_section: "Box 5", pdf_terminal: "blue (permit)" }` (or similar) to each rule. Three wins: (a) the landing-page writeup gets credible methodology to point to, (b) future regulatory updates can be diffed against the PDF more easily, (c) discrepancies surfaced during the planned `Quiz scenario accuracy deep dive` and `PDF text fidelity audit` get a clear authority to argue against. Cheap to add now while the connections are fresh from the recent Box 5/8/9 fixes; expensive to backfill later.

- **Rule-set versioning + last-reviewed date**: Add a top-level `version:` field to `nasfaa_rules.yml` (semver or yyyy.mm.dd) plus an optional `last_reviewed:` per rule. Surface the rule-set version in the page footer next to the build SHA, and in the planned About page's methodology section. Reasoning: regulations change, and users need to know whether what they're looking at reflects current law. Pairs with the formal-disclaimer / about-page work below.

- **Formal disclaimer + `/about/` page**: "For Entertainment Purposes Only" is the right tone for the chrome but doesn't carry enough weight for federal-regulation subject matter. Build a single-page `/nasfaa/about/` covering: what this is, methodology (rule extraction process, exhaustive cross-verification), sources (NASFAA PDF + IRC + HEA + FERPA with eCFR / Cornell LII links), last-reviewed date, who built it, how to report errors. Could absorb the "extended writeup" / landing-page work (gate #2). Establishes legal posture without making the chrome heavier than it should be.

- **Trim the quiz reveal box height**: The 2026-05-24 case-study rewrite added a `result_context.flavor` paragraph plus 1–3 case-study entries (title + summary + ↗ domain link) to every scenario reveal, which inflated the reveal box by 10–20 lines per question — it now overflows the viewport on most laptops. Acceptable for v0.1.0 but needs to land before declaring the unification gate done. Remediation candidates: (a) **Progressive disclosure** — show flavor + first case by default; add a `[c] cases` prompt that expands the rest in place (mirrors the existing dev-mode pattern). (b) **Collapsed-by-default** — render only the citation + score; a single `[c] context` prompt expands the entire flavor+cases section. (c) **Tighter case display** — drop summaries on the reveal, show only `• Title (year)  ↗ domain`; full summary appears on hover or in a follow-up `[s] sources` view. (d) **Move out of the box** — render flavor + cases as a separate, scrollable section *below* the reveal box so the box itself stays compact and the user can scroll if they want depth. (e) **Cap cases at 1** in the reveal, with a `[c] more` if there are additional ones. Option (a) or (d) is the most likely landing spot; (d) is the only one that keeps the entire reveal "above the fold."
