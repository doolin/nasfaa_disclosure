...
- **Extract quiz citation linker into its own module**: `linkifyCitation` + `escapeHtml` + `CITATION_BODIES` + `REF_RE` currently live inside the `app.js` IIFE and aren't exported, so the Node test (`web/quiz/test-citation.mjs`) duplicates the implementation. Extract to `web/quiz/citation.js` (IIFE + `module.exports`), have `app.js` consume via `window.NasfaaCitation`, and the test file then imports the real implementation. Eliminates drift risk between the tested impl and the live impl. The 21 existing tests pin the contract, so the extraction is safe to verify by running `make test`.
