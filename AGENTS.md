...
- `bin/` — `nasfaa` CLI plus stdlib-only tooling
  (`coverage-survey`, `time-analysis`, `timeline`). `bin/timeline`
  classifies every commit into a development phase (ordered ruleset +
  override table for judgment calls; unmatched commits reported as
  unclassified, never force-fit) and emits the README "## Timeline" table.
