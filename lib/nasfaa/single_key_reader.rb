...
module Nasfaa
  ...
  #
  # In single-key mode (when @input responds to :getch), read_char returns one
  # character at a time. Valid characters are echoed with a newline; unrecognized
  # characters (including ^M and other control chars) are consumed silently.
  # Ctrl-C (\x03) and Ctrl-\ (\x1c) are mapped to 'q'.
