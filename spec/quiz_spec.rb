...
RSpec.describe Nasfaa::Quiz do
  ...
  describe 'score tracking' do
    it 'tracks running score after each question' do
      ...
      expect(output.scan('Score:').length).to eq(24)
    end
  ...
  describe 'single-key mode' do
    ...
    it 'accepts single-character p/d input for all 24 scenarios' do
      answers = Array.new(24, 'p')
  ...
  describe 'banner' do
    ...
    it 'describes scenario mode in the banner' do
      answers = Array.new(24, 'permit')
      _, _, output, = run_quiz(answers)
      expect(output).to include('24 real-world scenarios')
    end
